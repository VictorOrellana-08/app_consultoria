...

{
    ...
    public function confirmedDeletion()
    {
        ...
        if (!$permission) {
            $this->alert('warning', 'Permiso no encontrado.');
            return;
        }
        ...
        if ($rolesCount > 0) {
            $this->alert('warning', 'No se puede eliminar este permiso porque tiene roles asignados.');
            return;
        }

        $permission->delete();
        $this->alert('success', 'El permiso ha sido eliminado.');
    }
